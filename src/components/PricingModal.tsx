...
const PricingModal: React.FC<PricingModalProps> = ({ onClose }) => {
    ...
    const handleUpgrade = async (plan: typeof PLANS[number]) => {
        if (!user) {
            alert('アップグレードするには、まずログインしてください。');
            return;
        }
        if (!supabase) {
                                ...
                                <div key={plan.id} className={`bg-white rounded-2xl border-2 ${plan.borderColor} shadow-xl overflow-hidden relative`}>
                                    ...
                                    <div className="p-6">
                                        ...
                                        <button
                                            ...
                                            className={`w-full py-4 bg-gradient-to-r ${plan.color} text-white rounded-xl font-bold hover:shadow-lg transition-all active:scale-[0.98] flex items-center justify-center gap-2 disabled:opacity-60`}
                                        >
                                            {loadingPlan === plan.id ? (
